...
ECHO;
ECHO 入力ファイル存在確認
IF NOT EXIST "narration\1.入力テキスト_sjis.txt"  ECHO "Not Found Input File! narration\1.入力テキスト_sjis.txt"
IF NOT EXIST "narration\1.入力テキスト_sjis.txt"  GOTO BYE
ECHO OK

:API
ECHO;
ECHO API(free,google,watson,azure,nict,winos,macos)選択（入力無しはfree）
SET api=
SET /P api="free,google,watson,azure,nict,winos,macos："
IF %api%@==@        SET api=free
