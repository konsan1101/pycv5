    ...
    while (True):
        ...
        if (cn_r.qsize() > 0):
            ...
            if (mode_get == 'PASS'):
                ...
                # 実行状況確認
                wBusyChange = False
                check = qFunc.busyCheck(qBusy_a_ctrl, 0)
                if (check != wBusyCtrl):
                    wBusyCtrl   = check
                    wBusyChange = True
                check = qFunc.busyCheck(qBusy_a_inp, 0)
                if (check != wBusyInput):
                    wBusyInput  = check
                    wBusyChange = True
                check = qFunc.busyCheck(qBusy_a_STT, 0)
                if (check != wBusySTT):
                    wBusySTT    = check
                    wBusyChange = True
                check = qFunc.busyCheck(qBusy_a_TTS, 0)
                if (check != wBusyTTS):
                    wBusyTTS    = check
                    wBusyChange = True
                check = qFunc.busyCheck(qBusy_a_play, 0)
                if (check != wBusyPlay):
                    wBusyPlay   = check
                    wBusyChange = True
                # 文字列を描画する
